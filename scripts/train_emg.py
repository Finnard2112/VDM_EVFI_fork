...
from src.dataset_emg_mp4 import (
    DEFAULT_EMG_SAMPLES_PER_INTERVAL,
    EMG_CHANNELS,
    load_validation_clip,
    make_datasets,
)

from src.models.unet_spatio_temporal_condition_fullControlnet import UNetSpatioTemporalConditionControlNetModel
from src.pipelines.pipeline_stable_video_diffusion_fullControlnet_emg_train import StableVideoDiffusionPipelineControlNet
...
def export_to_gif(frames, output_gif_path, fps):
    ...
    gif_path = output_gif_path if output_gif_path.endswith(".gif") else output_gif_path.replace(".mp4", ".gif")
...
def validate_once(val_save_dir, accelerator, pipeline, args, global_step, valid_dataset, record_idx=0, start_idx=0):
    ...
    if accelerator.mixed_precision == "fp16":
        ...
        valid_emg = valid_emg.to(dtype=torch.bfloat16)
    valid_emg = valid_emg.to(accelerator.device)
    ...
    for val_img_idx in range(args.num_validation_images):
        ...
        if args.save_validation_gifs:
            prefix = os.path.join(val_save_dir, f"{record.sequence_name}_step_{global_step}_val_{val_img_idx}")
            export_to_gif(video_frames, f"{prefix}_pred.gif", record.fps)
            export_to_gif(gt_frames, f"{prefix}_gt.gif", record.fps)
            export_to_gif(make_side_by_side_frames(video_frames, gt_frames), f"{prefix}_side_by_side.gif", record.fps)
    return video_frames, gt_frames
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=100,
        help="Number of random clip samples drawn from each MP4 per epoch.",
    )
    parser.add_argument(
        "--samples_per_folder",
        type=int,
        default=None,
        help="Deprecated alias for --samples_per_video.",
    )
    parser.add_argument(
        "--emg_samples_per_interval",
        type=int,
        default=DEFAULT_EMG_SAMPLES_PER_INTERVAL,
    )
    parser.add_argument("--emg_fs", type=float, default=500.0)
    parser.add_argument("--validation_start_idx", type=int, default=0)
    ...
    if args.samples_per_folder is not None:
        args.samples_per_video = args.samples_per_folder
...
def main():
    ...
    if args.controlnet_model_name_or_path:
        ...
        controlnet = ControlNetSDVModel.from_unet(
            unet,
            conditioning_channels=EMG_CHANNELS,
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.is_main_process:
                    ...
                    if global_step % args.validation_steps == 0:
                        ...
                        valid_save_dir = os.path.join(val_save_dir, "valid")
                        os.makedirs(valid_save_dir, exist_ok=True)

                        video_frames_valid, gt_frames_valid = validate_once(
                            val_save_dir=valid_save_dir,
                            accelerator=accelerator,
                            pipeline=pipeline,
                            args=args,
                            ...
                            record_idx=0,
                            start_idx=args.validation_start_idx,
                        )

                        psnr_valid = calculate_psnr(video_frames_valid, gt_frames_valid)
                        ssim_valid = calculate_ssim(video_frames_valid, gt_frames_valid)
                        lpips_valid = calculate_lpips(video_frames_valid, gt_frames_valid)
