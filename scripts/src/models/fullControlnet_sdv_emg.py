...
from src.dataset_emg_mp4 import EMG_CHANNELS, DEFAULT_EMG_SAMPLES_PER_INTERVAL

EV_CHANNELS = EMG_CHANNELS
...
class ControlNetConditioningEmbeddingSVD(nn.Module):
    def __init__(
        ...
        conditioning_channels: int = EMG_CHANNELS,
        ...
        self.emg_samples_per_interval = emg_samples_per_interval
        self.conv_in = nn.Conv1d(conditioning_channels, block_out_channels[0], kernel_size=5, padding=2)
        self.blocks = nn.ModuleList([])
        for i in range(len(block_out_channels) - 1):
            channel_in = block_out_channels[i]
            channel_out = block_out_channels[i + 1]
            self.blocks.append(nn.Conv1d(channel_in, channel_in, kernel_size=3, padding=1))
            self.blocks.append(nn.Conv1d(channel_in, channel_out, kernel_size=3, padding=1, stride=2))
        self.pool = nn.AdaptiveAvgPool1d(1)
        self.norm = nn.LayerNorm(block_out_channels[-1])
        self.conv_out = zero_module(
            nn.Linear(block_out_channels[-1], conditioning_embedding_channels)
        )

    def forward(self, conditioning, spatial_shape):
        if conditioning.dim() != 4:
            raise ValueError(
                f"EMG conditioning must have shape (batch, frames, channels, samples), got {conditioning.shape}"
            )
        batch_size, frames, channels, samples = conditioning.size()
        if channels != self.conditioning_channels:
            raise ValueError(
                f"Expected {self.conditioning_channels} EMG channels, got {channels}"
            )
        ...
        embedding = embedding[:, :, None, None].expand(-1, -1, height, width)
        return embedding
...
class ControlNetSDVModel(ModelMixin, ConfigMixin, FromOriginalControlNetMixin):
    ...
    def forward(
        ...
        if controlnet_cond is not None:
            ...
            controlnet_cond = self.controlnet_cond_embedding(controlnet_cond, sample.shape[-2:])
